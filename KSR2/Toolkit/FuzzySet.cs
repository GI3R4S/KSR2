...
namespace Toolkit
{
    ...
    {
        private readonly LinguisticVariablesGroup _linguisticVariableGroup;
        private readonly Dictionary<Record, double> _memberships = new Dictionary<Record, double>();
        private List<Record> _allElements;
        private List<Record> _elements;
        private FuzzySet _qualificator;


        public FuzzySet(IEnumerable<Record> elements, LinguisticVariable linguisticVariable)
        {
        ...
        private FuzzySet(IEnumerable<Record> elements, LinguisticVariablesGroup linguisticVariableGroup)
        {
            _elements = elements.ToList();
        ...
        public double this[Record item]
        {
            get
            {
        ...
        public IEnumerable<Record> Support => _elements.Where(record => this[record] > 0);
        ...
        public bool HasOr
        {
            get
            {
                ...
                while (current != null)
                {
                    if (current.RelationName == "lub") return true;
        ...
        {
            string output = "";
            LinguisticVariablesGroup currentGroup = _linguisticVariableGroup;
            if (Qualificator != null)
                output += $"podczas {Qualificator._linguisticVariableGroup.Variable.Name} występowały ";
            else
                output = " ";
        ...
        public double DegreeOfTruth(LinguisticVariable quantifier)
        {
            ...
            // Meassure T_3
            double t3 = Support.Count() / (double)_elements.Count;
            degrees.Add(t3);
